...
(function (global, factory) {
        ...
        if (node.hasAttributes()) {
          ...
          for (i = 0, len = attrMap.length; i < len; ++i) {
            ...
            output.push(
              ...
              attrNode.name,
              '="',
              escape$1(attrNode.value, ATTR_ENTITIES),
              '"',
            );
          }
        }
  ...
  function drawPath(waypoints, cornerRadius) {
    ...
    for (let i = 1; i < pointCount; i++) {
      ...
      const beforePoint2 = getPointAtLength(
        point,
        pointBefore,
        effectiveRadius * 0.5,
      );
      ...
      const afterPoint2 = getPointAtLength(
        point,
        pointAfter,
        effectiveRadius * 0.5,
      );
  ...
  function closest(element, selector, checkYourSelf) {
    ...
    return (
      (actualElement &&
        typeof actualElement.closest === "function" &&
        actualElement.closest(selector)) ||
      null
    );
  }
  ...
  function detect() {
    ...
    unbind$1 = window.removeEventListener
      ? "removeEventListener"
      : "detachEvent";
  ...
  function BpmnRenderer(
    ...
    function drawMarker(type, parentGfx, path, attrs) {
      return drawPath(
        parentGfx,
        path,
        assign$1({ "data-marker": type }, attrs),
      );
    }
    ...
    function renderEventIcon(element, parentGfx, attrs = {}, proxyElement) {
      ...
      if (
        semantic.get("eventDefinitions") &&
        semantic.get("eventDefinitions").length > 1
      ) {
        if (semantic.get("parallelMultiple")) {
          return eventIconRenderers["bpmn:ParallelMultipleEventDefinition"](
            parentGfx,
            nodeElement,
            attrs,
            isThrowing,
          ...
          return eventIconRenderers["bpmn:MultipleEventDefinition"](
            parentGfx,
            nodeElement,
            attrs,
            isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:MessageEventDefinition")) {
        return eventIconRenderers["bpmn:MessageEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:TimerEventDefinition")) {
        return eventIconRenderers["bpmn:TimerEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:ConditionalEventDefinition")) {
        return eventIconRenderers["bpmn:ConditionalEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:SignalEventDefinition")) {
        return eventIconRenderers["bpmn:SignalEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:EscalationEventDefinition")) {
        return eventIconRenderers["bpmn:EscalationEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:ErrorEventDefinition")) {
        return eventIconRenderers["bpmn:ErrorEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:CompensateEventDefinition")) {
        return eventIconRenderers["bpmn:CompensateEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
      ...
      if (isTypedEvent(semantic, "bpmn:TerminateEventDefinition")) {
        return eventIconRenderers["bpmn:TerminateEventDefinition"](
          parentGfx,
          nodeElement,
          attrs,
          isThrowing,
    ...
    function renderTaskMarkers(
      ...
      var loopCharacteristics = semantic.get("loopCharacteristics"),
        isSequential =
          loopCharacteristics && loopCharacteristics.get("isSequential");
      ...
      if (
        taskMarkers.includes("CompensationMarker") &&
    ...
    function renderAssociation(parentGfx, element, attrs = {}) {
      ...
      if (
        semantic.get("associationDirection") === "One" ||
    ...
    function renderDataObject(parentGfx, element, attrs = {}) {
      ...
      if (isCollection(semantic)) {
        var collectionPathData = pathMap.getScaledPath(
    ...
    function renderSubProcess(parentGfx, element, attrs = {}) {
      ...
      if (isEventSubProcess(element)) {
        ...
        if (!expanded) {
          ...
          var startEvents = flowElements.filter((e) =>
            is$1(e, "bpmn:StartEvent"),
          );

          if (startEvents.length === 1) {
            renderEventSubProcessIcon(
              startEvents[0],
              parentGfx,
              attrs,
              element,
    ...
    function renderEventSubProcessIcon(
      startEvent,
      parentGfx,
      attrs,
      proxyElement,
    ...
    var handlers = (this.handlers = {
      ...
      "bpmn:SequenceFlow": function (parentGfx, element, attrs = {}) {
        ...
        if (source) {
          ...
          if (
            semantic.get("conditionExpression") &&
            is$1(sourceSemantic, "bpmn:Activity")
          ) {
            attr$1(connection, {
              markerStart: marker(
                parentGfx,
          ...
          // default marker
          if (
            sourceSemantic.get("default") &&
            (is$1(sourceSemantic, "bpmn:Gateway") ||
              is$1(sourceSemantic, "bpmn:Activity")) &&
   ...
   * @return {SVGElement} mainGfx
   */
  BpmnRenderer.prototype.drawConnection = function (
    parentGfx,
    connection,
    attrs = {},
  ) {
  ...
  function getConnectionMid(connection) {
    ...
    var segmentProgress =
      (midLength - midSegment.startLength) / midSegment.length;

    var midPoint = {
      x:
        midSegment.start.x +
        (midSegment.end.x - midSegment.start.x) * segmentProgress,
      y:
        midSegment.start.y +
        (midSegment.end.y - midSegment.start.y) * segmentProgress,
    };
  ...
  BpmnImporter.prototype.add = function (semantic, di, parentElement) {
    ...
    if (is$1(di, "bpmndi:BPMNPlane")) {
      ...
      element = this._elementFactory.createRoot(
        elementData(semantic, di, attrs),
      );
  ...
  Overlays.prototype._addOverlay = function (overlay) {
    ...
    htmlContainer = domify$1(
      '<div class="djs-overlay" data-overlay-id="' + id + '">',
    );
      ...
      var path = businessObjectParents.flatMap(function (parent) {
        ...
        if (!parentPlane && is$1(parent, "bpmn:Process")) {
          var participant = elementRegistry.find(function (element) {
            ...
            return (
              businessObject && businessObject.get("processRef") === parent
            );
          });
   ...
   * @param {Shape} shape
   */
  DrilldownOverlayBehavior.prototype._updateDrilldownOverlay = function (
    shape,
  ) {
  ...
  DrilldownOverlayBehavior.prototype._addOverlay = function (element) {
    ...
    var button = domify$1(
        '<button type="button" class="bjs-drilldown">' +
          ARROW_DOWN_SVG +
          "</button>",
      ),
  ...
  function Injector(modules, _parent) {
    ...
    const providers = (this._providers = Object.create(
    ...
    function instantiate(type) {
      ...
      const Constructor = Function.prototype.bind.call(
        fn,
        null,
        ...dependencies,
      );
    ...
    function loadModule(moduleDefinition) {
      ...
      if (moduleExports) {
        ...
        const clonedModule = Object.keys(moduleDefinition).reduce(
          (clonedModule, key) => {
            if (
              key !== "__exports__" &&
              key !== "__modules__" &&
   ...
   * @private
   */
  DefaultRenderer.prototype.drawShape = function drawShape(
    visuals,
    element,
    attrs,
  ) {
  ...
  DefaultRenderer.prototype.drawConnection = function drawConnection(
    ...
    var line = createLine(
      ...
      assign$1({}, this.CONNECTION_STYLE, attrs || {}),
    );
    append(visuals, line);
  ...
  Canvas.prototype._fitViewport = function (center) {
    ...
    if (
      ...
      newViewbox = {
        x:
          inner.x + (center ? inner.width / 2 - outer.width / newScale / 2 : 0),
        y:
          inner.y +
  ...
  Properties.prototype.getProperty = function (target, name) {
    ...
    if (typeof strict !== "undefined") {
      const error = new TypeError(
        `unknown property <${name}> on <${target.$type}>`,
      );
    ...
    if (property && !property.isAttr) {
      ...
      if (serializationType) {
        ...
        if (elementTypeName) {
          ...
          const normalizedTypeName = normalizeTypeName(
            elementTypeName,
            node.ns,
            model,
          );
  ...
  Reader.prototype.fromXML = function (xml, options, done) {
    ...
    var context = new Context(
        assign$1({}, options, { rootHandler: rootHandler }),
      ),
    ...
    function resolveReferences() {
      ...
      for (i = 0; (r = references[i]); i++) {
        ...
        var property =
          getModdleDescriptor(element).propertiesByName[r.property];
  ...
  ElementSerializer.prototype.parseGenericNsAttributes = function (element) {
    return Object.entries(element)
      .filter(
        ([key, value]) =>
          !key.startsWith("$") && this.parseNsAttribute(element, key, value),
      )
  ...
  TypeSerializer.prototype.parseNsAttributes = function (element) {
    ...
    var attributes = ElementSerializer.prototype.parseNsAttributes
      .call(this, element)
      .filter((attr) => attr.name !== this.serialization);
  ...
  function Writer(options) {
    ...
    function toXML(tree, writer) {
      ...
      serializer
        .getNamespaces()
        .mapDefaultPrefixes(getDefaultPrefixMappings(model));
  ...
  function BpmnTreeWalker(handler) {
    ...
    this.handleDefinitions = function handleDefinitions(definitions, diagram) {
      ...
      if (
        ...
        is(rootElement, "bpmn:SubProcess")
      ) {
  ...
  function getDiagramsToImport(definitions, bpmnDiagram) {
    ...
    if (
      !is$1(bpmnElement, "bpmn:Process") &&
      !is$1(bpmnElement, "bpmn:Collaboration")
    ) {
  ...
  BaseViewer.prototype.importXML = async function importXML(xml, bpmnDiagram) {
    ...
    try {
      ...
      const importResult = await this.importDefinitions(
        definitions,
        bpmnDiagram,
      );
  ...
  BaseViewer.prototype.open = async function open(bpmnDiagramOrId) {
    ...
    const { warnings } = await importBpmnDiagram(
      this,
      definitions,
      bpmnDiagram,
    );
