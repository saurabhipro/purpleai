...
class PTLForm(models.Model):
    ...
    global_status = fields.Selection([
        ...
        ('handover', 'Handover')
    ], string='Global Status', default='ptl', tracking=True, store=True, readonly=False)
    ...
    #     for record in self:
    #         print("function is working",record.ptl_section_status)
    #         # Auto-compute global status based on section approvals
    #         if record.ptl_section_status == 'approved':
    #             print("if condiiotjlfj - ")
    #             record.global_status = 'form_verification'

    #         elif record.critical_path_section_status == 'pending':
    #             record.global_status = 'kick_off_meeting'
            # if record.conceptual_design_section_status == 'approved':
            #     record.global_status = 'handover'
            # if record.tenant_appointment_section_status == 'approved':
    ...
    def action_submit_critical_path_section(self):
        for rec in self:
            rec.critical_path_section_status = 'pending'
            rec.global_status = 'kick_off_meeting'

    def action_submit_tenant_app_section(self):
        for rec in self:
            rec.tenant_appointment_section_status = 'pending'

    def action_submit_conceptual_design_section(self):
        for rec in self:
            rec.conceptual_design_section_status = 'pending'
