...
openai_service.py
────────────────
OpenAI GPT provider, implements BaseAIService. Centralized version for ai_core.
"""
...
class OpenAIService(BaseAIService):
    ...
    def generate(
        ...
        *,
        ...
        # Enable JSON mode if the prompt mentions JSON (Only supported on gpt-4o, gpt-4-turbo, etc.)
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if resp.status_code == 429:
                    # Distinguish billing quota exhausted (non-retryable) from real rate-limiting (retryable)
                    try:
                    ...
                    if err_code == "insufficient_quota":
                        raise RuntimeError(
                            _("OpenAI quota exhausted: Your account has no remaining credits. "
...
def upload_file_to_openai(file_path: str, env=None, **kwargs) -> Dict[str, Any]:
