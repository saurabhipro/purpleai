...
mistral_service.py
──────────────────
Mistral AI provider, implements BaseAIService.

Reads configuration from Odoo system parameters:
    tender_ai.mistral_api_key
    tender_ai.mistral_default_model

Backward-compatible module-level function kept:
    generate_with_mistral(contents, model, temperature, env, max_retries)
"""
...
class MistralService(BaseAIService):
    ...
    def generate(
        ...
        *,
        ...
        if not api_key:
            raise RuntimeError(
                _("Mistral API Key is missing. Please configure it in Purple AI Settings.")
            )
        ...
        # Format messages for potential multi-modal input
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if resp.status_code == 429:
                    ...
                    sleep_time = min(5 * (attempt + 1), 60)  # 5s, 10s, 15s...
                    _logger.warning(
                        "MISTRAL API: rate-limited (429), attempt %d/%d. Waiting %ds...", 
                ...
                if resp.status_code != 200:
                    ...
                    raise RuntimeError(
                        _("Mistral AI returned error %s: %s") % (resp.status_code, resp.text)
                    )
        ...
        raise RuntimeError(
            _("Mistral AI call failed after %d attempts: %s") % (max_retries + 1, last_err)
        )
