...
Public surface that the rest of the codebase uses
--------------------------------------------------
Module-level convenience wrappers are kept for backward compatibility:

    generate_with_gemini(contents, ...)   → calls GeminiService().generate()
    upload_file_to_gemini(file_path, ...) → GeminiService().upload_file()
    get_gemini_api_key(env=None)          → GeminiService().get_api_key()
    get_configured_model(env=None)        → GeminiService().get_model()
    list_available_models(env=None)       → GeminiService().list_models()

All new code should import ``ai_service.generate()`` instead of calling
``generate_with_gemini`` directly.
"""
...
class GeminiService(BaseAIService):
    ...
    Supports file-upload caching, concurrency throttling, and SDK compat shims
    ...
    def generate(
        ...
        *,
        ...
        """
        Call Gemini generate_content() with automatic retry on 429/rate-limit.
        """
        if genai is None:
            raise RuntimeError(
            ...
            try:
                ...
                if genai_errors and isinstance(exc, genai_errors.ClientError):
                    if "429" in msg or "RESOURCE_EXHAUSTED" in msg:
                        ...
                        _logger.warning(
                            "GEMINI API: rate-limited, sleeping 5s before retry",
                        )
                        time.sleep(5)
                        last_err = exc
                        continue
    ...
    def list_models(self, env=None) -> List[str]:
        """Return model names available to the configured API key."""
        try:
            client = self._get_client(env=env)
    ...
    def upload_file(
        ...
        **kwargs,
        ...
        """
        Upload a local file (PDF, image …) to the Gemini Files API.

        Returns the File object (may not yet be ACTIVE if wait_active=False).

        NOTE: The SDK makes GET requests to refresh file metadata before each
        generate_content() call – this is expected behaviour, not a bug.
        """
    ...
    def get_api_key(self, env=None) -> str:
        ...
        Resolve the Gemini API key from (in priority order):
          1. Odoo config file (odoo.conf): ai_api_key / gemini_api_key
          2. Odoo system parameter: tender_ai.ai_api_key
          3. Environment variable: AI_API_KEY / GEMINI_API_KEY
    ...
    def get_model(self, env=None) -> str:
        """
        Resolve the model to use (priority order):
          1. Odoo system parameter: tender_ai.default_model
          2. Environment variables: AI_MODEL / AI_TENDER_MODEL / GEMINI_*
          3. Hardcoded fallback: gemini-2.5-flash
        """
        global _model_cache
    ...
    def invalidate_model_cache(self) -> None:
        """Invalidate the cached model name (call after changing settings)."""
        global _model_cache
        with _model_cache_lock:
            _model_cache = None
    ...
    def _get_client(self, env=None):
        """Thread-safe client getter (one client instance per thread)."""
        if genai is None:
            raise RuntimeError(
                "google-genai package is not installed. "
                "Install it with: pip install google-genai"
...
def _generate_content_compat(client, model: str, contents: Any, temperature: float) -> Any:
    """Compat shim: tries generation_config, then config, then bare call."""
    try:
...
def generate_with_gemini(
    ...
    """
    Backward-compatible wrapper around GeminiService.generate().
    Existing callers (client_master.py, res_config_settings.py, etc.) continue
...
def upload_file_to_gemini(
    ...
    """Backward-compatible wrapper around GeminiService.upload_file()."""
...
def get_gemini_api_key(env=None) -> str:
    """Backward-compatible wrapper around GeminiService.get_api_key()."""
    return _service.get_api_key(env=env)


def get_configured_model(env=None) -> str:
    """Backward-compatible wrapper around GeminiService.get_model()."""
    return _service.get_model(env=env)


def _invalidate_model_cache() -> None:
    """Backward-compatible wrapper around GeminiService.invalidate_model_cache()."""
    _service.invalidate_model_cache()


def list_available_models(env=None) -> List[str]:
    """Backward-compatible wrapper around GeminiService.list_models()."""
