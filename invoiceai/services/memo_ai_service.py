...
def call_azure_openai(env, prompt, settings=None, enforce_html=True):
    ...
    # Debug logging
    _logger.info(
        "Azure OpenAI call: endpoint=%s, key_present=%s, deployment=%s",
        endpoint or 'NOT SET',
    ...
    if not api_key or not endpoint:
        ...
        raise ValueError(
            f"Azure OpenAI credentials incomplete. Missing: {', '.join(missing)}. "
            "Go to Settings → General Settings → AI Core Configuration to configure Azure. "
            "Make sure to SAVE after entering the credentials."
        )
    try:
        ...
        # Note: Some Azure AI Services deployments don't support temperature parameter
        # Start without temperature to avoid errors, the model will use its default
        payload = {
        ...
        try:
            ...
            err_str = str(e)
            _logger.error("Azure OpenAI request failed: %s", err_str)
            # Re-raise with all details for debugging
            raise ValueError(f"Azure OpenAI API error: {err_str}") from e
        
        # Log response details
        finish_reason = response.choices[0].finish_reason if response.choices else None
        _logger.info("Azure response: finish_reason=%s, content_length=%d", 
                     finish_reason, len(response.choices[0].message.content or ''))
        
        text = response.choices[0].message.content
        if not text:
            _logger.warning("Azure returned empty content. Response: %s", response)
            text = ''
        else:
            text = text.strip()
        usage = response.usage
