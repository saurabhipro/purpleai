...
class InvoiceAIAPI(http.Controller):
    def _api_auth(self):
        ...
        if not expected:
            return True, None
        if not header:
            _logger.info('Invoice AI API: dev key required but X-AI-Core-Dev-Key header missing')
            return False, 'missing_header'
        if header != expected:
            _logger.info('Invoice AI API: dev key mismatch (header present, length=%s)', len(header))
    ...
    def _get_upload_file(self):
        ...
        if files:
            ...
            if first and getattr(first, 'filename', None):
                return first
        _logger.info('Invoice AI upload: no usable file; multipart keys=%s', list(files.keys()))
        return None
    ...
    @http.route('/invoiceai/v1/ping', type='http', auth='none', methods=['GET', 'OPTIONS'], csrf=False)
    def ping(self, **_kwargs):
        if request.httprequest.method == 'OPTIONS':
            return request.make_response('', headers=self._cors_headers(), status=204)
        return self._json_response({'ok': True, 'module': 'invoiceai'})

    @http.route(
        ['/invoiceai/v1/clients', '/invoiceai/v1/clients/'],
        type='http',
        auth='none',
        methods=['GET', 'OPTIONS'],
        csrf=False,
    ...
    @http.route('/invoiceai/v1/upload', type='http', auth='none', methods=['POST', 'OPTIONS'], csrf=False)
    def upload(self, **_kwargs):
        ...
        _logger.warning(
            'Invoice AI upload POST: content_type=%r content_length=%r file_keys=%s form_keys=%s',
        ...
        if not uploaded_file:
            _logger.warning(
                'Invoice AI upload → 400 no_file (multipart had no usable part). '
            ...
            return self._json_response(
                {
                    ...
                    'hint': 'No purple_ai.client with that id. Create one under Invoice AI → Client Master.',
                },
                status=400,
            )
        if not client:
            ...
            _logger.warning(
                'Invoice AI upload → 400 no_client (purple_ai.client count=%s). '
                'Create a client under Invoice AI → Client Master.',
                n,
            )
            return self._json_response(
                {
                    ...
                    'reason': 'no_client',
                    'hint': 'In Odoo open Invoice AI → Client Master (or Clients): create a client with '
                    'an Extraction Template. Set Root Folder Path in Invoice AI settings if required. '
    ...
    @http.route('/invoiceai/v1/results', type='http', auth='none', methods=['GET', 'OPTIONS'], csrf=False)
    ...
    @http.route('/invoiceai/v1/viewer_data/<int:res_id>', type='http', auth='none', methods=['GET', 'OPTIONS'], csrf=False)
